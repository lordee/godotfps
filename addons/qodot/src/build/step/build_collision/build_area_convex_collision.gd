...
func should_spawn_collision_shapes(entity_idx, entity_properties):
...
func _finalize(context) -> Dictionary:
	...
	for entity_key in area_collision_shapes:
		for brush_key in area_collision_shapes[entity_key]:
			...
			var properties = area_collision_shape_data['entity_properties']

			var brush_area = QodotArea.new()
			brush_area.properties = properties
